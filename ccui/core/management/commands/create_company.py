...
DEFAULT_TEST_MANAGER_ROLE_PERMISSIONS = set([
        "PERMISSION_COMPANY_INFO_VIEW",
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        manager_role.permissions = [
            p for p in PermissionList.get(auth=admin)
            if p.permissionCode in DEFAULT_TEST_MANAGER_ROLE_PERMISSIONS]
        ...
        print (
            ...
            "tester role id %s, manager role id %s, and admin role id %s."
            % (company.id, tester_role.id, manager_role.id, admin_role.id))
